...
function mk_apworld() {
    ...
    rsync --progress \
        ...
        "${root}/" "${destdir}/haste"
    pushd "${destdir}"
    zip -9r "haste.apworld" "haste"
    popd

    rm --force --recursive "${destdir}/haste"
}
...
function cp_data() {
    ...
    # cp --verbose ${root}/RandomizerAP.us.gci ${destdir}
    cp --verbose "${root}/haste.yaml" ${destdir}
}
...
function main() {
    ...
    local bundle_base="haste_apworld"
